...
fn bench_try_acquire_success(c: &mut Criterion) {
    ...
    group.bench_function("single_token", |b| {
        ...
        let mut limiter = SlidingWindowCounter::new(u32::MAX, 1);
...
fn bench_try_acquire_rejected(c: &mut Criterion) {
    ...
    group.bench_function("full_window", |b| {
        let mut limiter = SlidingWindowCounter::new(0, 60);
        b.iter(|| black_box(limiter.try_acquire(black_box(1))));
    });
    group.finish();
}
...
fn bench_refresh(c: &mut Criterion) {
    ...
    for &window in &[1u64, 10, 60] {
        group.bench_with_input(
            ...
            &window,
            |b, &window| {
                let mut limiter = SlidingWindowCounter::new(u32::MAX, window);
                b.iter(|| {
                    limiter.refresh();
                    black_box(&limiter);
                });
...
fn bench_getters(c: &mut Criterion) {
    ...
    let limiter = {
        let mut l = SlidingWindowCounter::new(1_000, 60);
        l.try_acquire(250);
        l
    };
...
fn bench_shared_uncontended(c: &mut Criterion) {
    ...
    group.bench_function("try_acquire", |b| {
        let limiter = SlidingWindowCounterShared::new(u32::MAX, 1);
        b.iter(|| black_box(limiter.try_acquire(black_box(1))));
    });
    group.finish();
}
...
fn bench_shared_contended(c: &mut Criterion) {
    ...
    for &threads in &[2usize, 4, 8] {
        ...
        group.bench_with_input(
            ...
            |b, &threads| {
                b.iter_custom(|iters| {
                    let limiter = Arc::new(SlidingWindowCounterShared::new(u32::MAX, 1));
