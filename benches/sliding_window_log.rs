...
fn bench_try_acquire_success(c: &mut Criterion) {
    let mut group = c.benchmark_group("SlidingWindowLog/try_acquire_success");
...
fn bench_refresh(c: &mut Criterion) {
    let mut group = c.benchmark_group("SlidingWindowLog/refresh");
    for &window in &[0u64, 1, 60] {
...
fn bench_shared_contended(c: &mut Criterion) {
    let mut group = c.benchmark_group("SlidingWindowLogShared/contended");
